...
/**
 * Read-only paywall management view: every wallet on the platform, its
 * tier and subscription status. Never errors on an empty or missing
 * `subscriptions` table — see lib/server/subscribers.ts.
 */
export async function GET(request: Request) {
  try {
    ...
    console.error(
      "Admin subscribers listing failed unexpectedly.",
      error instanceof Error ? (error.stack ?? error.message) : error,
    );
    return NextResponse.json({ error: "Subscriber data could not be loaded. Try again." }, { status: 500, headers: NO_STORE_HEADERS });
  }
}
