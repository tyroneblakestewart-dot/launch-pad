...
// "Disconnect X" (issue #335): deletes the stored, encrypted access token
// outright. Wallet-signed via the shared challenge flow (POST
// /api/social/challenge, purpose "social:x-disconnect") so only the wallet
// that owns a connection can remove it.
...
export async function POST(request: Request) {
  ...
  await getSocialConnectionsStore().delete(authorisation.walletAddress, "x");
  await recordAdminActivityBestEffort({
    kind: "social-x-disconnected",
    serviceKey: "social-posting",
    message: `Wallet ${authorisation.walletAddress} disconnected X.`,
  });
