...
import { getTokenTradesCacheHealth, type TokenTradesCacheHealth } from "@/lib/server/token-trades-rpc";
import {
...
export type TokenLaunchesPipelineDeps = {
  ...
  /** Overridable for tests; defaults to lib/server/token-trades-rpc.ts's real cache health. */
  readTokenTradesCacheHealth?: (now?: number) => TokenTradesCacheHealth;
};
...
/**
 * Reports on lib/server/token-trades-rpc.ts's own health (issue #430) —
 * mirrors curveProgressReadStage exactly. Amber until the cache has ever
 * been warmed — it only warms lazily, on GET /api/token-trades's first
 * live read, so a freshly started server or a token that hasn't had its
 * page viewed yet has nothing to report.
 */
function tradesReadStage(
  readHealth: (now?: number) => TokenTradesCacheHealth,
  now: number,
): AdminPipelineStage {
  const health = readHealth(now);
  if (health.lastReadAt === null) {
    return stage(
      ...
      "amber",
      "No trade-history read has been attempted yet — the cache warms on a token page's first request.",
    );
  }
...
export async function buildTokenLaunchesPipeline(
  ...
  const readTokenTradesCacheHealth = deps.readTokenTradesCacheHealth ?? getTokenTradesCacheHealth;
  const tradesStage = tradesReadStage(readTokenTradesCacheHealth, Date.now());
