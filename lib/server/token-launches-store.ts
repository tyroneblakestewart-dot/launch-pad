...
export interface TokenLaunchesStore {
  ...
  /** Case-insensitive lookup of the earliest recorded launch's created_at for a given (chainId, curveAddress), or null when Postgres is unconfigured or no record exists — used to derive a real trade-scan start block instead of a historical eth_getCode search the pruned Robinhood RPC can't serve (issue #433). */
